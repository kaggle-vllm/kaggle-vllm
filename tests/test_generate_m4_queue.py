...
def test_queue_preserves_negative_compatibility_and_historical_shards() -> None:
    ...
    assert queue["queued_shards"] == 30
    assert queue["queued_serving_cells"] == 360
    assert queue["review_required_shards"] == 2
    assert queue["remaining_shards"] == 32
    assert queue["skipped_gemma_shards"] == 15
    assert queue["next_shard_id"] == "qwen25_3b-prefill_heavy-r02"
