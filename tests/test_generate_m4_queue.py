...
def test_queue_preserves_negative_compatibility_and_historical_shards() -> None:
    ...
    assert queue["preserved_shards"] == 28
    assert queue["preserved_serving_cells"] == 336
    assert queue["queued_shards"] == 29
    assert queue["queued_serving_cells"] == 348
    assert queue["review_required_shards"] == 3
    assert queue["remaining_shards"] == 32
