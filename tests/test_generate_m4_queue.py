...
def test_queue_preserves_negative_compatibility_and_historical_shards() -> None:
    ...
    assert queue["preserved_shards"] == 25
    assert queue["preserved_serving_cells"] == 300
    assert queue["queued_shards"] == 33
    assert queue["queued_serving_cells"] == 396
    assert queue["review_required_shards"] == 2
    assert queue["remaining_shards"] == 35
    assert queue["skipped_gemma_shards"] == 15
    assert queue["next_shard_id"] == "ministral3_3b_bf16-short-r02"
