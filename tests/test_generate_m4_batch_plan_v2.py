...
def test_continuations_are_model_scoped_repetition_blocked_and_complete() -> None:
    ...
    assert plan["preserved_logical_shards"] == 41
    assert plan["remaining_logical_shards"] == 19
    assert plan["continuation_executable_shards"] == 15
    ...
    assert plan["batch_ids"][0] == "r03-llama"
    ...
    assert len(shard_ids) == len(set(shard_ids)) == 15
...
def test_first_continuation_is_exact_llama_r03_block() -> None:
    ...
    assert first["batch_id"] == "r03-llama"
