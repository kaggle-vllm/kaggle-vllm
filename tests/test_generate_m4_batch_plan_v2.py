...
def test_continuations_are_model_scoped_repetition_blocked_and_complete() -> None:
    ...
    assert plan["preserved_logical_shards"] == 36
    assert plan["remaining_logical_shards"] == 24
    assert plan["continuation_executable_shards"] == 21
    assert plan["review_required_shard_ids"] == [
        "qwen25_3b-prefill_heavy-r00",
        "qwen25_3b-prefill_heavy-r01",
        "qwen25_3b-prefill_heavy-r02",
    ]
    assert len(plan["batches"]) == 7
    assert plan["batch_ids"][0] == "r03-qwen"
    ...
    assert len(shard_ids) == len(set(shard_ids)) == 21
...
def test_first_continuation_is_exact_qwen_r03_block() -> None:
    ...
    assert first["batch_id"] == "r03-qwen"
