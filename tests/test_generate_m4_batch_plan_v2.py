...
def test_continuations_are_model_scoped_repetition_blocked_and_complete() -> None:
    ...
    assert plan["preserved_logical_shards"] == 3
    assert plan["remaining_logical_shards"] == 57
    assert plan["continuation_executable_shards"] == 56
    assert plan["review_required_shard_ids"] == [
        "qwen25_3b-prefill_heavy-r00"
    ]
    assert len(plan["batches"]) == 19
    assert plan["batch_ids"][0] == "fill-r00-phi"
...
def test_first_continuation_is_exact_phi_r00_group() -> None:
    ...
    assert first["batch_id"] == "fill-r00-phi"
