...
def test_promoted_canonical_batch_cannot_be_rescheduled() -> None:
    ...
    with pytest.raises(ResearchEvidenceError, match="unknown or duplicate"):
        select_batch(remaining, "r03-llama")
    with pytest.raises(ResearchEvidenceError, match="unknown or duplicate"):
        select_batch(remaining, "r04-phi")
    stale_batch = {
        "ordered_shard_ids": [
    ...
    stale_phi = {
        ...
        "execution_order": [
            {"shard_id": "phi4_mini-prefill_heavy-r02"},
            {"shard_id": "phi4_mini-short-r02"},
            {"shard_id": "phi4_mini-balanced-r02"},
        ],
        "already_completed_skips": [],
        "review_required_exclusions": [],
    }
    with pytest.raises(ResearchEvidenceError, match="refusing to reschedule settled"):
        validate_batch_against_queue(stale_phi, queue)
...
def test_phi_r02_batch_is_settled_and_non_runnable() -> None:
    evidence = json.loads((ROOT / "research/M4_EVIDENCE_STATUS.json").read_text())
    attempt = evidence["principal_batch_attempts"]["r02-phi-attempt-1"]
    assert attempt["session_id"] == "m4-r02-phi-20260916T141636Z-6f8069da"
...
def test_llama_r03_batch_is_settled_and_non_runnable() -> None:
    evidence = json.loads((ROOT / "research/M4_EVIDENCE_STATUS.json").read_text())
    attempt = evidence["principal_batch_attempts"]["r03-llama-attempt-1"]
    assert attempt["session_id"] == "m4-r03-llama-20260920T095447Z-7d3fc1d4"
    assert attempt["completed_canonical_shards"] == [
...
def test_phi_r04_batch_is_settled_and_non_runnable() -> None:
    evidence = json.loads((ROOT / "research/M4_EVIDENCE_STATUS.json").read_text())
    attempt = evidence["principal_batch_attempts"]["r04-phi-attempt-1"]
    assert attempt["session_id"] == "m4-r04-phi-20260920T193618Z-c949f541"
    assert attempt["completed_canonical_shards"] == [
...
def test_v12_freeze_matches_generator_and_clean_notebook() -> None:
    ...
    assert tracked["protocol_amendment_version"] == "M4-BATCH-3"
    assert tracked["batch_notebook_source_digest"] == (
        "4518f4fcecc8e94f1593a0abe3b570d88cbdc05ebd144ac0b38bef6bcce20805"
    )
...
def test_v13_freeze_matches_generator_and_clean_notebook() -> None:
    tracked = json.loads(
        (ROOT / "research/M4_BATCH_SOURCE_FREEZE_V13.json").read_text()
    )
    assert build_freeze_v13(ROOT) == tracked
    assert build_freeze_v13(ROOT) == tracked
    assert verify_batch_source_freeze(
        ROOT, ROOT / "research/M4_BATCH_SOURCE_FREEZE_V13.json"
    ) == tracked
    assert tracked["protocol_amendment_version"] == "M4-BATCH-3"
    assert tracked["batch_notebook_source_digest"] == (
        "da7f5454982cc3ff4e556291d13366bf544241a62196b991dee751b85299a891"
    )
    assert tracked["implementation_source_commit"] == (
        "571bd07f478f820813ff6c9b19faf3470b138fb5"
    )
    assert tracked["notebook_pin_commit"] == (
        "40c7c9da88bce712e3a87bcf9b5af52793cf6aa8"
    )


def test_v14_freeze_matches_generator_and_clean_notebook() -> None:
    tracked = json.loads(
        (ROOT / "research/M4_BATCH_SOURCE_FREEZE_V14.json").read_text()
    )
    assert build_freeze_v14(ROOT) == tracked
    assert build_freeze_v14(ROOT) == tracked
    assert verify_batch_source_freeze(
        ROOT, ROOT / "research/M4_BATCH_SOURCE_FREEZE_V14.json"
    ) == tracked
    assert tracked["protocol_amendment_version"] == "M4-BATCH-3"
    assert tracked["batch_notebook_source_digest"] == (
...
def test_v15_freeze_matches_generator_and_clean_notebook() -> None:
    ...
    assert tracked["protocol_amendment_version"] == "M4-BATCH-3"
    assert tracked["batch_notebook_source_digest"] == (
        "a20604a8bc8a88e64b38bf39e239c0e1aeb11e5514a684b0e290ca6382bdd483"
    )
    assert tracked["implementation_source_commit"] == (
...
def test_v16_freeze_matches_generator_and_clean_notebook() -> None:
    tracked = json.loads(
        (ROOT / "research/M4_BATCH_SOURCE_FREEZE_V16.json").read_text()
    )
    assert build_freeze_v16(ROOT) == tracked
    assert build_freeze_v16(ROOT) == tracked
    assert verify_batch_source_freeze(
        ROOT, ROOT / "research/M4_BATCH_SOURCE_FREEZE_V16.json"
    ) == tracked
    assert tracked["protocol_amendment_version"] == "M4-BATCH-3"
    assert tracked["batch_notebook_source_digest"] == (
        "74397a450c8484f8095c16c895752330f464e8d25eadefdcff84e01199addc8f"
    )
    assert tracked["implementation_source_commit"] == (
        "45e1da603e4f27dca67168a9d4058c50a6c60556"
    )
    assert tracked["notebook_pin_commit"] == (
        "da1395dd1c9e98968e3a29766069203be82aa4b6"
    )


def test_v17_freeze_matches_generator_and_clean_notebook() -> None:
    tracked = json.loads(
        (ROOT / "research/M4_BATCH_SOURCE_FREEZE_V17.json").read_text()
    )
    assert build_freeze_v17(ROOT) == tracked
    assert build_freeze_v17(ROOT) == tracked
    assert verify_batch_source_freeze(
        ROOT, ROOT / "research/M4_BATCH_SOURCE_FREEZE_V17.json"
    ) == tracked
    assert tracked["protocol_amendment_version"] == "M4-BATCH-3"
    assert tracked["batch_notebook_source_digest"] == (
...
def test_v18_freeze_matches_generator_and_clean_notebook() -> None:
    ...
    assert tracked["protocol_amendment_version"] == "M4-BATCH-3"
    assert tracked["batch_notebook_source_digest"] == (
        "194b3dc641a66cd7c705f5dad1358242ac8a8217d298076fc41f31d5c1694851"
    )
    assert tracked["implementation_source_commit"] == (
...
def test_v19_freeze_matches_generator_and_clean_notebook() -> None:
    tracked = json.loads(
        (ROOT / "research/M4_BATCH_SOURCE_FREEZE_V19.json").read_text()
    )
    assert build_freeze_v19(ROOT) == tracked
    assert build_freeze_v19(ROOT) == tracked
    assert verify_batch_source_freeze(
        ROOT, ROOT / "research/M4_BATCH_SOURCE_FREEZE_V19.json"
    ) == tracked
    assert tracked["protocol_amendment_version"] == "M4-BATCH-3"
    assert tracked["batch_notebook_source_digest"] == (
...
def test_v21_freeze_matches_final_commit_consistent_notebook() -> None:
    tracked = json.loads(
        (ROOT / "research/M4_BATCH_SOURCE_FREEZE_V21.json").read_text()
    )
    assert build_freeze_v21(ROOT) == tracked
    assert build_freeze_v21(ROOT) == tracked
    assert verify_batch_source_freeze(
        ROOT, ROOT / "research/M4_BATCH_SOURCE_FREEZE_V21.json"
    ) == tracked
    notebook = ROOT / "kaggle-notebooks/kaggle_vllm_m4_execute_batch.ipynb"
    identity = validate_batch_notebook_commit_consistency(ROOT, notebook)
    ...
    assert tracked["notebook_pin_commit"] == (
        "989093672b94d3e6fc040fdcf6a2800b1d7c4fe2"
    )
