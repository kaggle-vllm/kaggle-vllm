...
def test_gemma_canonical_negative_remains_auditable_and_blocks_principal():
    ...
    assert status["principal_preserved_shards"] == 18
    assert status["principal_remaining_shards"] == 42
    assert status["principal_review_required_shards"] == 1
    assert status["next_principal_shard"] == "ministral3_3b_bf16-balanced-r01"
