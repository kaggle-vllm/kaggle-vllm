...
def test_gemma_canonical_negative_remains_auditable_and_blocks_principal():
    ...
    assert status["principal_preserved_shards"] == 46
    assert status["principal_remaining_shards"] == 14
    assert status["principal_review_required_shards"] == 5
    assert status["principal_queued_shards"] == 9
    assert status["next_principal_shard"] == "phi4_mini-balanced-r04"
