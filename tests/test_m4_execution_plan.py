...
def test_gemma_canonical_negative_remains_auditable_and_blocks_principal():
    ...
    assert status["principal_preserved_shards"] == 44
    assert status["principal_remaining_shards"] == 16
    assert status["principal_review_required_shards"] == 4
    assert status["principal_queued_shards"] == 12
    assert status["next_principal_shard"] == "qwen25_3b-short-r04"
