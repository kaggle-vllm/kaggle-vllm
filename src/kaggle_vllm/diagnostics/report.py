...
def generate_markdown_report(
    m1_dir: str, m2_dir: str, output_path: str | None = None
) -> str:
    ...
    for label, res in m1_results.items():
        est = res["estimate"]
        alpha_str = (
            f"{est.inferred_alpha_us_per_step:.2f} µs"
            if est.inferred_alpha_us_per_step > 0
            else "N/A"
        )
        md.append(
            f"| `{label}` | {est.measured_tp1_tok_s:.1f} | {est.measured_tp2_tok_s:.1f} | {est.observed_delta_percent:+.1f}% | {alpha_str} | {est.explanation} |"
        )
    ...
    for cell in m2_results:
        ...
        md.append(
            f"| {cell.concurrency} | {cell.tp1_throughput_tok_s:.2f} | {cell.tp2_throughput_tok_s:.2f} | {est.observed_delta_percent:+.1f}% | {ttft1} | {ttft2} | {regime} |"
        )
    ...
    md.append(
        "1. **Measured vs. Inferred:** Throughput and latencies are measured quantities from JSON evidence. Step alpha ($\alpha$) and bus bandwidth ($\beta$) are inferred analytical parameters."
    )
    md.append(
